...
// items listed
$total_listed = 0;
$list_query = $conn->prepare("SELECT COUNT(*) FROM products_sell WHERE seller_id = ?");
if ($list_query) {
...
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id']) && is_numeric($_GET['id']) && isset($_GET['confirm']) && $_GET['confirm'] == 'yes') {
    ...
    $check_query = $conn->prepare("SELECT seller_id FROM products_sell WHERE product_sell_id = ?");
    ...
    if ($check_query->num_rows > 0) {
        ...
        if ($seller_id == $user_id) {
            // Delete the product
            $delete_query = $conn->prepare("DELETE FROM products_sell WHERE product_sell_id = ?");
...
$active_query = $conn->prepare("
    SELECT product_sell_id, product_name, product_price, created_at 
    FROM products_sell 
    ...
    <header>
        <nav class="navbar">
            ...
            <ul class="nav-links">
                ...
                <li><a href="../pages/userDash.php"><i class="fa fa-user"></i></a></li>
            </ul>
        </nav>
    </header>
    ...
    <div class="userDashSec">
        <div class="container">
            ...
            <main class="main-content">
                ...
                <!-- Listings Management -->
                <div class="listings-purchase-wrapper">
                <section class="card">
                                ...
                                <tbody>
                                    <?php foreach ($active_listings as $listing): ?>
                                        <tr>
                                            ...
                                            <td>
                                                <a href="sell.php?id=<?= $listing['product_sell_id'] ?>" class="action-btn">Edit</a>
                                                <a href="#" class="action-btn delete-btn" 
                                                data-id="<?= $listing['product_sell_id'] ?>" 
