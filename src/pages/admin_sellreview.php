...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['product_id']) && isset($_POST['action'])) {
    ...
    if ($product_id && in_array($action, ["approve", "reject", "reset"])) {
        ...
        $stmt = $conn->prepare("UPDATE products_sell SET product_status = ?, updated_at = NOW() WHERE product_sell_id = ?");
...
$count_pending = $conn->query("SELECT COUNT(*) as count FROM products_sell WHERE product_status = '" . STATUS_PENDING . "'")->fetch_assoc()['count'];
$count_approved = $conn->query("SELECT COUNT(*) as count FROM products_sell WHERE product_status = '" . STATUS_APPROVED . "'")->fetch_assoc()['count'];
$count_rejected = $conn->query("SELECT COUNT(*) as count FROM products_sell WHERE product_status = '" . STATUS_REJECTED . "'")->fetch_assoc()['count'];

$pending_products = $conn->query("SELECT * FROM products_sell WHERE product_status = '" . STATUS_PENDING . "' ORDER BY created_at DESC LIMIT $offset, $records_per_page");
$approved_products = $conn->query("SELECT * FROM products_sell WHERE product_status = '" . STATUS_APPROVED . "' ORDER BY created_at DESC LIMIT $offset, $records_per_page");
$rejected_products = $conn->query("SELECT * FROM products_sell WHERE product_status = '" . STATUS_REJECTED . "' ORDER BY created_at DESC LIMIT $offset, $records_per_page");
...
$update_empty = $conn->query("UPDATE products_sell SET product_status = '" . STATUS_PENDING . "' 
...
<body>
    <div class="container">
        ...
        <?php if (isset($_SESSION['message'])): ?>
            <div class="flash-message <?= isset($_SESSION['message_type']) ? $_SESSION['message_type'] : 'success' ?>">
